...
#include "src/core/ext/transport/winpipe/winpipe_transport.h"
...
#define WINPIPE_LOG(...)                                          \
  do {                                                           \
    if (GRPC_TRACER_ON(grpc_winpipe_trace)) gpr_log(__VA_ARGS__); \
  } while (0)

#define WINPIPE_LOG_LAST_ERROR()                                 \
  do {                                                           \
    if (GRPC_TRACER_ON(grpc_winpipe_trace)) {                    \
      char* last_error = GetLastErrorString();                   \
...
typedef struct winpipe_transport {
  grpc_transport base;
  shared_mu *mu;
  gpr_refcount refs;
  bool is_client;
  ...
  bool is_closed;
  // struct winpipe_transport *other_side;
  struct winpipe_stream *stream_list;
} winpipe_transport;
...
} winpipe_slice_byte_stream;


typedef struct winpipe_stream {
  winpipe_transport *t;
  ...
  bool listed;
  struct winpipe_stream *stream_list_prev;
  struct winpipe_stream *stream_list_next;
} winpipe_stream;
...
static winpipe_read* winpipe_read_malloc(
  winpipe_slice_byte_stream* stream,
  grpc_closure* on_complete)
{
  winpipe_read* read = (winpipe_read*)malloc(sizeof(winpipe_read));
  if (NULL == read) {
    return NULL;
  }
...
static grpc_error *winpipe_slice_byte_stream_pull(grpc_exec_ctx *exec_ctx,
  grpc_byte_stream *bs,
  grpc_slice *slice) {
  winpipe_slice_byte_stream *stream = (winpipe_slice_byte_stream *)bs;
  ...
  *slice = grpc_slice_buffer_take_first(&stream->slice_buffer);
  return GRPC_ERROR_NONE;
}

static void winpipe_slice_byte_stream_shutdown(grpc_exec_ctx *exec_ctx,
  grpc_byte_stream *bs,
  grpc_error *error) {
  winpipe_slice_byte_stream *stream = (winpipe_slice_byte_stream *)bs;
...
static void winpipe_slice_byte_stream_destroy(grpc_exec_ctx *exec_ctx,
  grpc_byte_stream *bs) {
  winpipe_slice_byte_stream *stream = (winpipe_slice_byte_stream *)bs;
  grpc_slice_buffer_destroy(&stream->slice_buffer);
  GRPC_ERROR_UNREF(stream->shutdown_error);
}

static const grpc_byte_stream_vtable winpipe_slice_byte_stream_vtable = {
  winpipe_slice_byte_stream_next, winpipe_slice_byte_stream_pull,
  winpipe_slice_byte_stream_shutdown, winpipe_slice_byte_stream_destroy };

void winpipe_slice_byte_stream_init(winpipe_slice_byte_stream *s,
  sb_list_entry *le) {
  s->base.length = (uint32_t)le->sb.length;
  s->base.flags = 0;
  s->base.vtable = &winpipe_slice_byte_stream_vtable;
  s->le = le;
  s->shutdown_error = GRPC_ERROR_NONE;
}

static void ref_transport(winpipe_transport *t) {
  WINPIPE_LOG(GPR_DEBUG, "ref_transport %p", t);
  gpr_ref(&t->refs);
}

static void really_destroy_transport(grpc_exec_ctx *exec_ctx,
  winpipe_transport *t) {
  WINPIPE_LOG(GPR_DEBUG, "really_destroy_transport %p", t);
...
static void unref_transport(grpc_exec_ctx *exec_ctx, winpipe_transport *t) {
  WINPIPE_LOG(GPR_DEBUG, "unref_transport %p", t);
  if (gpr_unref(&t->refs)) {
...
static void ref_stream(winpipe_stream *s, const char *reason) {
  WINPIPE_LOG(GPR_DEBUG, "ref_stream %p %s", s, reason);
  STREAM_REF(s->refs, reason);
}

static void unref_stream(grpc_exec_ctx *exec_ctx, winpipe_stream *s,
  const char *reason) {
  WINPIPE_LOG(GPR_DEBUG, "unref_stream %p %s", s, reason);
  STREAM_UNREF(exec_ctx, s->refs, reason);
}

static void really_destroy_stream(grpc_exec_ctx *exec_ctx, winpipe_stream *s) {
  WINPIPE_LOG(GPR_DEBUG, "really_destroy_stream %p", s);
...
static void log_metadata(const grpc_metadata_batch *md_batch, bool is_client,
  ...
  for (grpc_linked_mdelem *md = md_batch->list.head; md != NULL;
    ...
    gpr_log(GPR_INFO, "winpipe:%s:%s: %s: %s", is_initial ? "HDR" : "TRL",
...
static grpc_error *fill_in_metadata(grpc_exec_ctx *exec_ctx, winpipe_stream *s,
...
static int init_stream(grpc_exec_ctx *exec_ctx, grpc_transport *gt,
  ...
  WINPIPE_LOG(GPR_DEBUG, "init_stream %p %p %p", gt, gs, server_data);
  winpipe_transport *t = (winpipe_transport *)gt;
  winpipe_stream *s = (winpipe_stream *)gs;
  s->arena = arena;

  s->refs = refcount;
  // Ref this stream right now
  ref_stream(s, "winpipe_init_stream:init");
  ...
  gpr_mu_lock(&t->mu->mu);
  s->listed = true;
  ref_stream(s, "winpipe_init_stream:list");
  ...
  if (!server_data) {
    ref_transport(t);
    winpipe_transport *st = t->other_side;
    ref_transport(st);
    s->other_side = NULL;  // will get filled in soon
                           // Pass the client-side stream address to the server-side for a ref
    ref_stream(s, "winpipe_init_stream:clt");  // ref it now on behalf of server
                                              // side to avoid destruction
    WINPIPE_LOG(GPR_DEBUG, "calling accept stream cb %p %p",
    ...
    winpipe_stream *cs = (winpipe_stream *)server_data;
    s->other_side = cs;
    // Ref the server-side stream on behalf of the client now
    ref_stream(s, "winpipe_init_stream:srv");
...
static void close_stream_locked(grpc_exec_ctx *exec_ctx, winpipe_stream *s) {
  if (!s->closed) {
    ...
    if (s->listed) {
      winpipe_stream *p = s->stream_list_prev;
      winpipe_stream *n = s->stream_list_next;
      if (p != NULL) {
...
static void close_other_side_locked(grpc_exec_ctx *exec_ctx, winpipe_stream *s,
  const char *reason) {
...
static void fail_helper_locked(grpc_exec_ctx *exec_ctx, winpipe_stream *s,
  grpc_error *error) {
  WINPIPE_LOG(GPR_DEBUG, "read_state_machine %p fail_helper", s);
  ...
  if (!s->trailing_md_sent) {
    ...
    winpipe_stream *other = s->other_side;
  ...
  if (s->recv_message_op) {
    WINPIPE_LOG(GPR_DEBUG, "fail_helper %p scheduling message-ready %p", s,
      error);
    GRPC_CLOSURE_SCHED(
      exec_ctx, s->recv_message_op->payload->recv_message.recv_message_ready,
      GRPC_ERROR_REF(error));
    if (s->recv_message_op != s->recv_trailing_md_op) {
      WINPIPE_LOG(GPR_DEBUG, "fail_helper %p scheduling message-on-complete %p",
        s, error);
...
static void read_state_machine(grpc_exec_ctx *exec_ctx, void *arg,
  ...
  WINPIPE_LOG(GPR_DEBUG, "read_state_machine %p", arg);
  winpipe_stream *s = (winpipe_stream *)arg;
  ...
  if (!slice_buffer_list_empty(&s->to_read_message) && s->recv_message_op) {
    winpipe_slice_byte_stream_init(
    ...
    *s->recv_message_op->payload->recv_message.recv_message =
    ...
    WINPIPE_LOG(GPR_DEBUG, "read_state_machine %p scheduling message-ready", s);
    GRPC_CLOSURE_SCHED(
      exec_ctx, s->recv_message_op->payload->recv_message.recv_message_ready,
      GRPC_ERROR_NONE);
    if (s->recv_message_op != s->recv_trailing_md_op) {
  ...
  if (s->to_read_trailing_md_filled) {
    ...
    if (s->recv_message_op != NULL) {
      ...
      // satisfied
      WINPIPE_LOG(GPR_DEBUG, "read_state_machine %p scheduling message-ready",
        s);
      GRPC_CLOSURE_SCHED(
        exec_ctx,
  ...
  if (s->recv_message_op || s->recv_trailing_md_op) {
    ...
    // rescheduled
    WINPIPE_LOG(GPR_DEBUG, "read_state_machine %p still needs closure %p %p", s,
...
static bool cancel_stream_locked(grpc_exec_ctx *exec_ctx, winpipe_stream *s,
  grpc_error *error) {
  bool ret = false;  // was the cancel accepted
  WINPIPE_LOG(GPR_DEBUG, "cancel_stream %p with %s", s,
...
static void perform_stream_op(grpc_exec_ctx *exec_ctx, grpc_transport *gt,
  ...
  WINPIPE_LOG(GPR_DEBUG, "perform_stream_op %p %p %p", gt, gs, op);
  winpipe_stream *s = (winpipe_stream *)gs;
  ...
  if (op->cancel_stream) {
    ...
    WINPIPE_LOG(GPR_DEBUG, "perform_stream_op %p%s%s%s%s%s%s", s,
  ...
  if (error == GRPC_ERROR_NONE &&
    ...
    if (error == GRPC_ERROR_NONE && op->send_initial_metadata) {
      ...
      if (*destfilled || s->initial_md_sent) {
        ...
        WINPIPE_LOG(GPR_DEBUG, "Extra initial metadata %p", s);
    ...
    if (error == GRPC_ERROR_NONE && op->send_trailing_metadata) {
      ...
      if (*destfilled || s->trailing_md_sent) {
        // The buffer is already in use; that's an error!
        WINPIPE_LOG(GPR_DEBUG, "Extra trailing metadata %p", s);
  ...
  if (error == GRPC_ERROR_NONE &&
    ...
    WINPIPE_LOG(GPR_DEBUG, "perform_stream_op %p scheduling on_complete %p", s,
      error);
...
static void close_transport_locked(grpc_exec_ctx *exec_ctx,
  winpipe_transport *t) {
  WINPIPE_LOG(GPR_DEBUG, "close_transport %p %d", t, t->is_closed);
...
static void perform_transport_op(grpc_exec_ctx *exec_ctx, grpc_transport *gt,
  ...
  winpipe_transport *t = (winpipe_transport *)gt;
  WINPIPE_LOG(GPR_DEBUG, "perform_transport_op %p %p", t, op);
  gpr_mu_lock(&t->mu->mu);
...
static void destroy_stream(grpc_exec_ctx *exec_ctx, grpc_transport *gt,
  ...
  WINPIPE_LOG(GPR_DEBUG, "destroy_stream %p %p", gs, then_schedule_closure);
  winpipe_stream *s = (winpipe_stream *)gs;
...
static void destroy_transport(grpc_exec_ctx *exec_ctx, grpc_transport *gt) {
  winpipe_transport *t = (winpipe_transport *)gt;
  WINPIPE_LOG(GPR_DEBUG, "destroy_transport %p", t);
  gpr_mu_lock(&t->mu->mu);
  close_transport_locked(exec_ctx, t);
  gpr_mu_unlock(&t->mu->mu);
...
void grpc_winpipe_transport_init(void) {
  ...
  g_fake_auth_value = grpc_slice_from_static_string("winpipe-fail");
  grpc_exec_ctx_finish(&exec_ctx);
}

static const grpc_transport_vtable winpipe_vtable = {
  sizeof(winpipe_stream), "winpipe",        init_stream,
  set_pollset,           set_pollset_set, perform_stream_op,
  perform_transport_op,  destroy_stream,  destroy_transport,
  get_endpoint };

/*******************************************************************************
* Main winpipe transport functions
*/
static void winpipe_transports_create(grpc_exec_ctx *exec_ctx,
  grpc_transport **server_transport,
  const grpc_channel_args *server_args,
  grpc_transport **client_transport,
  const grpc_channel_args *client_args) {
  WINPIPE_LOG(GPR_DEBUG, "winpipe_transports_create");
  winpipe_transport *st = (winpipe_transport *)gpr_zalloc(sizeof(*st));
  winpipe_transport *ct = (winpipe_transport *)gpr_zalloc(sizeof(*ct));
  // Share one lock between both sides since both sides get affected
  st->mu = ct->mu = (shared_mu *)gpr_malloc(sizeof(*st->mu));
  gpr_mu_init(&st->mu->mu);
  gpr_ref_init(&st->mu->refs, 2);
  st->base.vtable = &winpipe_vtable;
  ct->base.vtable = &winpipe_vtable;
...
grpc_channel *grpc_winpipe_channel_create(grpc_server *server,
  ...
  void *reserved) {
  GRPC_API_TRACE("grpc_winpipe_channel_create(server=%p, args=%p)", 2,
    (server, args));
  ...
  default_authority_arg.value.string = (char *)"winpipe.authority";
  grpc_channel_args *client_args =
    grpc_channel_args_copy_and_add(args, &default_authority_arg, 1);

  grpc_transport *server_transport;
  grpc_transport *client_transport;
  winpipe_transports_create(&exec_ctx, &server_transport, server_args,
    &client_transport, client_args);

  grpc_server_setup_transport(&exec_ctx, server, server_transport, NULL,
    server_args);
  grpc_channel *channel =
    grpc_channel_create(&exec_ctx, "winpipe", client_args,
